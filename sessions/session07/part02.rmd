...
- 15 278 packages sur le CRAN
- objectif : creer le package mathr avec une fonction.
...
Commençons par créer l'arborescence du package nommé `{mathr}`

--

```{r echo=TRUE, eval=FALSE}
path <- "/Users/nicolascasajus/Desktop/mathr"
...
## 2. Initialisation du versioning
...
Le fichier DESCRIPTION s'ouvre dans votre éditeur de texte et affiche :
...
`r icon::fa("hand-point-right")` Par convention, la première version d'un package en développement est habituellement notée : `0.0.0.9000`



---

## 4. Choix d'une licence

Définissons maintenant la licence du package qui imposera ses conditions d'utilisation.
Nous choisissons ici la licence [**MIT**](https://choosealicense.com/licenses/mit/) très peu contraignante.
...
Pour vous orienter dans votre choix : [https://choosealicense.com](https://choosealicense.com)

---

## 5. Mise en place de la documentation
...
Générons la documentation du fichier `R/mathr-package.R`
...
Contenu du fichier `man/mathr-package.Rd`

```{r echo=FALSE, eval=TRUE}
setwd(path)
cat(paste0(readLines("man/mathr-package.Rd"), collapse = "\n"))
setwd(opath)
```

---

## 6. Développement d'une fonction R

Créons une première fonction R : `moyenne()`. Tout d'abord, créons le fichier.
...
## 7. Documentation de la fonction

Une fonction n'est rien sans son mode d'emploi. Et de toute façon, c'est obligatoire.
...
- `@import` : indique les packages à importer (évite d'utiliser `library()`)
- `@importFrom` : indique les fonctions des dépendances à importer


---

## 8. Compilation de la documentation

Générons maintenant le fichier `Rd`.
...
## 10. Vérification du package

Notre package est maintenant prêt. Vérifions-le:
...
── R CMD check results ─────────────────────────────────── mathr 0.0.0.9000 ────
Duration: 3m 9.9s

0 errors ✔ | 0 warnings ✔ | 0 notes ✔
```

`r icon::fa("beer", size = 2)` &nbsp;Notre package ne comporte aucune erreur !!!


---

## 11. Changement de version

Notre package est vérifié et a passé avec succès tous les tests. Nous pouvons changer sa version.
...
## 12. Installation du package

Puisque notre package est vérifié, nous pouvons maintenant l'installer.
...
```
* installing *source* package ‘mathr’ ...
...
Ainsi, nous pouvons y accéder via la fonction `library()`
...
```
mathr-package              package:mathr               R Documentation

mathr: Structure d'un package simple sous R

Description:

     Le package ‘mathr’ illustre la structure d'un package...
...
```
moyenne                 package:mathr                  R Documentation
...
```{r echo=TRUE, eval=FALSE}
mathr::moyenne(c(2, 4, NA), na_rm = TRUE)
```
...
```{r echo=TRUE, eval=FALSE}
usethis::create_package("path/to/pkgname")      # Package skeleton
...
usethis::use_r("funname")                       # Edit new function + doc
